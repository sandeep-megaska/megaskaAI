...
import { buildMotionPlan } from "@/lib/video/motion/buildMotionPlan";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const motionPlan = buildMotionPlan({
      ...
      hasMandatoryAnchors: protectedCoreFlow.hasAllMandatoryAnchors,
      firstFrameUrl: firstFrame.url ?? firstFrameUrl,
      lastFrameUrl: lastFrame.url ?? lastFrameUrl,
      firstFrameGenerationId: firstFrame.generationId,
      lastFrameGenerationId: lastFrame.generationId,
    });
    for (const warning of [...motionPlan.motionWarnings, ...motionPlan.anchorGapWarnings]) {
      pushCompatibilityWarning(compatibilityWarnings, warning);
    }
    ...
    if (decompositionEnabled) {
      ...
      const generationInsertPayload = {
        ...
        video_meta: {
          ...
          motionRiskLevel,
          motionPlanVersion: motionPlan.motionPlanVersion,
          motionCategory: motionPlan.motionCategory,
          primaryAction: motionPlan.primaryAction,
          secondaryAction: motionPlan.secondaryAction,
          actionCount: motionPlan.actionCount,
          motionDrivenByAnchorsExpected: motionPlan.motionDrivenByAnchorsExpected,
          expectedAnchorBehavior: motionPlan.expectedAnchorBehavior,
          suggestedAnchorPattern: motionPlan.suggestedAnchorPattern,
          anchorSuitabilityStatus: motionPlan.anchorSuitabilityStatus,
          motionWarnings: motionPlan.motionWarnings,
          anchorGapLevel: motionPlan.anchorGapLevel,
          anchorMotionPatternGuess: motionPlan.anchorMotionPatternGuess,
          anchorGapWarnings: motionPlan.anchorGapWarnings,
          motionDiagnostics: motionPlan.motionDiagnostics,
          actionSpecificAnchorRecommended: motionPlan.actionSpecificAnchorRecommended,
          multiAnchorSequenceRecommended: motionPlan.multiAnchorSequenceRecommended,
          futureSuggestedAnchorStates: motionPlan.futureSuggestedAnchorStates,
          compatibilityWarnings,
          protectedCoreFlowEnabled: protectedCoreFlow.protectedCoreFlowEnabled,
