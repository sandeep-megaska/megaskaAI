...
export default function LookbookPage() {
  ...
  const [outputStyle, setOutputStyle] = useState<"catalog" | "studio" | "lifestyle">("catalog");
    ...
    <main className="min-h-screen bg-zinc-950 px-6 py-10 text-zinc-100">
      <div className="mx-auto max-w-6xl space-y-8">
        <section className="rounded-xl border border-white/10 bg-zinc-900/50 p-6">
          ...
          <p className="mt-2 text-sm text-zinc-400">Generate a catalog-consistent 6-shot photoset with locked model + garment identity packs.</p>
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-2">
            ...
            <label className="space-y-1 text-sm">
              <span className="text-zinc-300">Output style</span>
              <select value={outputStyle} onChange={(event) => setOutputStyle(event.target.value as "catalog" | "studio" | "lifestyle")} className="w-full rounded-lg border border-white/10 bg-zinc-950/70 px-3 py-2">
          ...
          <div className="mt-6 rounded-lg border border-white/10 bg-zinc-950/60 p-4">
            <h2 className="text-sm font-semibold text-zinc-200">Shot pack preview</h2>
            <div className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
              {DEFAULT_SHOT_PACK.map((shot) => (
          ...
          <button type="button" onClick={generateLookbook} disabled={submitting || !modelId || !garmentId || !backendId} className="mt-6 rounded-lg bg-indigo-500 px-4 py-3 text-sm font-semibold text-white disabled:opacity-50">
            {submitting ? "Generating lookbook..." : "Generate Lookbook"}
          </button>
