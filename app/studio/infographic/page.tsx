...
import { CREATIVE_TEMPLATES, MEGASKA_BRAND_KIT } from "@/lib/infographic/templates";
import {
...
export default function InfographicStudioPage(){
  ...
  const applyTemplate=(templateId:string)=>{const template=CREATIVE_TEMPLATES.find(t=>t.id===templateId);if(!template)return;commit(structuredClone(template.document));setPreset("amazon-square");setSelectedIds([]);setStatus(`${template.name} loaded. Every object is editable.`);};
