...
export function assignRolesFromCandidates(input: { candidates: PackReuseCandidate[]; motionPrompt: string }) {
  ...
  assigned.push({ role: "front", generation_id: primary.generation_id, source_kind: "reused", confidence_score: primary.score });
  assigned.push({ role: "fit_anchor", generation_id: primary.generation_id, source_kind: "reused", confidence_score: Number((primary.score - 0.02).toFixed(4)) });
  ...
  if (left) {
    assigned.push({ role: "three_quarter_left", generation_id: left.generation_id, source_kind: "reused", confidence_score: left.score });
  } else {
    assigned.push({
  ...
  if (right) {
    assigned.push({ role: "three_quarter_right", generation_id: right.generation_id, source_kind: "reused", confidence_score: right.score });
  } else {
    assigned.push({
